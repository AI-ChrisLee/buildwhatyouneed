-- ========================================
-- COMPLETE RESET - DISABLE TRIGGERS FIRST
-- ========================================

-- STEP 1: Disable the trigger that's causing conflicts
...
-- STEP 3: Create test users WITHOUT trigger interference
-- ========================================
...
-- Add paid member to leads and stripe
INSERT INTO public.leads (email, name, stage, user_id)
...
SELECT 
  c.id, l.title, l.video_id, l.source_url, l.ord
FROM courses c
CROSS JOIN (VALUES
  (1, 'Day 1: CRM Replacement', 'wistia123', 'https://github.com/aichrislee/crm', 1),
  (1, 'Day 2: Email Tool Build', 'wistia456', 'https://github.com/aichrislee/email', 2),
  (2, 'Day 8: Analytics Dashboard', 'wistia789', 'https://github.com/aichrislee/analytics', 1)
) AS l(course_ord, title, video_id, source_url, ord)
...
SELECT 
  ...
  'Threads: ' || (SELECT count(*) FROM threads) as counts;
